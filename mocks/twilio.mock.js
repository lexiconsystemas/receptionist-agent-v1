/**
 * Twilio Mock API
 * Simulates Twilio responses for development/testing
 *
...
async function mockSendSms(to, body, options = {}) {
  if (!MOCK_ENABLED) {
    throw new Error('Mocks disabled - configure real TWILIO_AUTH_TOKEN');
  }
  ...
  setTimeout(() => {
    ...
    logger.info('[MOCK] SMS delivered', { messageSid });
  }, 1000);
...
/**
 * Mock TwiML generation for RetellAI connection
 */
...
<Response>
  ...
  logger.info('[MOCK] Generated TwiML for RetellAI connection');
...
async function mockMakeCall(to, options = {}) {
  ...
  logger.info('[MOCK] Call initiated', { callSid, to });
...
function mockValidateWebhookSignature(signature, url, params) {
  ...
  logger.warn('[MOCK] Skipping Twilio webhook signature validation');
  return true;
}
...
function clearMockStores() {
  ...
  logger.info('[MOCK] Mock stores cleared');
}
